...
const DesignerBoard = (props) => {
    const editorRef = useRef(null);//编辑器ref对象
    //添加控件
    useEffect(() => {
        if (props.controlType) {
            switch (props.controlType.split("_")[0]) {
                case "input": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">文本框</span>');
                    break;
                case "richInput": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:150px;height:100px;" contenteditable="false">多行文本框</span>');
                    break;
                case "radio": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">单选</span>');
                    break;
                case "checkBox": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">复选</span>');
                    break;
                case "listBox": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">下拉选择器</span>');
                    break;
                case "compute": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">计算控件</span>');
                    break;
                case "macro": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false" disabled="disabled">宏控件</span>');
                    break;
                case "detailTable": editorRef.current.editor.execCommand('mceInsertContent', false, '<table id="' + props.controlType + '" style="font-size:16px;" class="mceTmpl" width="100%" data-field="detail-table" border="1" cellspacing="0" cellpadding="0"><tr><th scope="col"> </th><th scope="col"> </th></tr><tr><td> </td><td> </td></tr></table>');
                    break;
                case "treeListBox": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:22px;" contenteditable="false">级联选择器</span>');
                    break;
                case "image": editorRef.current.editor.execCommand('mceInsertContent', false, '<span id="' + props.controlType + '" class="mceTmpl" style="width:100px;height:100px;" contenteditable="false">图片</span>');
                    break;
            }
        }
    }, [props.controlType]);
    ...
    >
        <Editor
            ...
            init={{
                ...
                language: 'zh_CN',
                extended_valid_elements: 'span[disabled|id|class|style|contenteditable]',//设置合法的属性（不被过滤掉）
            }}
