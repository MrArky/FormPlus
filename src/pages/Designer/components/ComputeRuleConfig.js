...
import { Button, Modal, message, Tree, Tag, Tooltip, Typography } from 'antd';
        ...
        >
            <div style={{ display: "flex", height: 400 }}>
                <div style={{ width: 200, borderRight: "1px solid #efefef" }}>
                    <Tree treeData={treeData} height={400} defaultExpandAll selectedKeys={selectedKeys} onSelect={(selectedKeys, e) => {
                        if (e.node.key != "全部" && e.node.key.split("_")[0] != "detailTable") {
                            if (props.onChange && typeof props.onChange === "function") {
                                let computeControl=props.controlsAttrs.find(c=>c.name==props.controlName);
                                props.onChange([...props.value, {
                                    key: new Date().valueOf(),
                                    type: "control",
                                    label: e.node.describe,
                                    value: e.node.name,
                                    summary:computeControl.belongTo==e.node.belongTo && computeControl.belongTo!="window" && e.node.tableField && computeControl.tableField?false:true,
                        ...
                        onDoubleClick={()=>{
                            ...
                            if(computeControl.belongTo==c.belongTo && computeControl.belongTo != "window" && c.tableField && computeControl.tableField){
                                if (props.onChange && typeof props.onChange === "function") {
                                    ...
                                    let tag=props.value.find(fc => fc.key == c.key);
                                    tag.summary=!tag.summary;
                                    props.onChange(value);
                                }
                            }
                        }}
                        >{c.label}{c.summary?"列汇总值":""}</Tag>)}
                    </div>
